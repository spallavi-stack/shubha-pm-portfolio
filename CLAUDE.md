...
**Separate environment limitation: this session's PDF-extraction tooling is broken.** poppler-utils, pypdf, and pdfminer.six all failed when a July 2026 verification pass tried to read a linked Ofgem PDF for the SIF's RIIO-3 funding figures — install and library-load attempts failed across the board, so that figure stayed tagged as an unverified Assumption in `grounding-research.md` for a tooling reason, not because the data doesn't exist. Check whether this is still broken before assuming a PDF-only primary source is unreachable on a future project.

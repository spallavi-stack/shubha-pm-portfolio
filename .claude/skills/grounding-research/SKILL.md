...
2c. **For any product whose core value is a calculation, score, match, or algorithmic "check," separate two different things in the technical/economic section: aggregate research figures, and per-user runtime inputs.** Grounding research is naturally good at the first (a price cap, a typical system cost, a rate range — all population-level numbers with a citeable source) and can silently skip the second entirely. SunnySideUp's first pass sourced solid ranges for electricity price, SEG rates, system cost, and payback period, but never once asked what an actual user's calculator session would need as input to produce their specific number (household consumption, occupancy pattern, current tariff) or where each input would come from (self-report, bill upload, third-party API). This was caught only after the product brief was otherwise finished, by the user, not by this process. Before closing out a technical/economic section for a product with a calculation at its core, explicitly list the runtime inputs the product needs, separate from the aggregate figures — even if the honest answer for several is "no source decided yet, flag in the product brief's Open Questions." Don't wait for `research-auditor` or the user to notice this gap; it belongs here, at research time.

2d. **Before writing a new numeric claim, check whether the same fact already has a value stated elsewhere in the project** (other docs in `projects/<slug>/docs/`, `index.html`, `prototype.html`) **or elsewhere in the portfolio** (e.g. `docs/video-scripts.md`). If a different value already exists, don't silently pick one, even the one you just found a source for — surface both to the user and ask which is canonical, the same protocol `research-auditor` step 2a runs at audit time. This caught a real case late (Flexy's US electricity bill increase was 26% on the case study page and 40% in the product brief, unnoticed until the user asked); catching it here, before the second number gets written, is cheaper than catching it later.

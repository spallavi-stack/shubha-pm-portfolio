...
- A concrete go/no-go test — adapt a real methodology (e.g. a Sean-Ellis-style "would be very disappointed" signal) to synthetic personas/interviews honestly, stating plainly that it's a synthetic proxy, not real user data. **If using a Sean-Ellis-style disappointment question before any product exists, that methodology is a retention test built for people with actual usage history — asking a persona "how would you feel if this didn't exist" about a product they've never used is incoherent on its face.** Describe the concept to the persona within the interview itself first (the one-line pitch plus Solution), then ask the disappointment question anchored to that description. That's a concept test, not a literal PMF survey, and the classic 40% threshold it borrows from was calibrated on real usage data, not a reaction to a pitch — say so explicitly rather than presenting the resulting percentage as equivalent-strength evidence.
...
- **Success metrics & validation methodology** — the section built specifically to answer "how would you actually test this later," per Sean Ellis's product-market-fit survey methodology (the 40%-"very disappointed" test, refined from Dropbox/Eventbrite/LogMeIn data) adapted honestly for a portfolio context with no real users. That adaptation itself needed a correction on SunnySideUp: the first version asked synthetic personas how they'd feel if the product didn't exist without ever describing the product to them first, i.e. applying a retention test built for actual product usage to personas who'd never encountered SunnySideUp at all. Fixed to require describing the concept within the interview before asking the disappointment question, an explicit concept test rather than a literal PMF survey.

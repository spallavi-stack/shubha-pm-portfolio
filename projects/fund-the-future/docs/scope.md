...
**Selection method and full working: [`market-selection.md`](market-selection.md).** The first version of this scope named Kenya, the Philippines, and India, then swapped India for Maldives. Those were chosen by association rather than against criteria, so the selection was redone properly: hard filters first (foreign donations legally receivable, a working payout rail, no sanctions or conflict blocking payouts), then six ranked criteria (climate impact, registered climate NGO count with an informal-sector flag, poverty as both a rate and an absolute count, diaspora and remittance corridor strength, whether a public NGO registry can actually be queried, and how established the country's payout rail is). Payout rail counts twice on purpose: pass or fail on whether any documented rail reaches the country, and ranked on how established it is, since cross-border money movement is the product's defining technical constraint.

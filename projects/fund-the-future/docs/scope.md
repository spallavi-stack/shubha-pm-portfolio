...
- **Target user (donor side):** Climate-motivated retail donors, meaning individuals who already give to environmental or climate causes and would give to adaptation work if it were legible, specific and trustworthy. Larger organisational and philanthropic funders are treated as visitors who arrive through a shared campaign link rather than through any discovery feature on the platform. **Decided August 2026**, replacing an earlier framing built on diaspora giving, which conflated sending money to one's own family with donating to an organisation.
...
1. **Geography:** Global South organisations raising primarily from donors in higher-income markets. Cross-border money movement is therefore the defining technical and regulatory constraint of the product.
...
**Selection method and the full working: [`market-selection.md`](market-selection.md).** Hard filters first (foreign donations legally receivable, a working payout rail, no sanctions or conflict blocking payouts), then five ranked criteria: climate impact, registered climate NGO count with an informal-sector flag, poverty as both a rate and an absolute count, whether a public NGO registry can actually be queried, and how established the country's payout rail is.

**A sixth criterion, diaspora size and remittance corridor strength, was deleted in August 2026 and every market re-scored without it.** It rested on treating remittance as evidence of willingness to donate. Sending money to one's own family is an obligation to a known person. Donating to a climate organisation is charitable giving to strangers. Proximity to a country is not proximity to a cause, and a market selected on remittance strength was selected on the wrong evidence.

**Why the Philippines, of the three:**

- **It is strongest on the two criteria that map to the product's two hard problems.** Its SEC register carries an annual filing obligation covering General Information Sheets and audited financial statements, so verification has something continuing to check rather than a one-off record of existence. Its payout rail reaches both banks and seven wallet providers, through two like-for-like providers, so an organisation with no bank account can still be paid.
...
- Market sizing must not stop at historical crowdfunding volumes for climate causes. Per the `grounding-research` standing rule, it also needs comparable-market analogs (disaster-relief giving, foundation funding for adaptation and resilience, general-purpose crowdfunding volumes in the pilot market) and an addressable-population estimate for how many organisations of this type plausibly exist.
...
8. **Climate-motivated retail donors behave like the donors in the available benchmark data.** Every per-donor figure in the research, covering gift size, conversion, retention and channel, comes from general US and Western online fundraising across all causes. Whether donors to environmental and climate causes differ on any of those dimensions was not established.

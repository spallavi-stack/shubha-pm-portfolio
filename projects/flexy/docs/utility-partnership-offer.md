...
## One-liner
Flexy gives ComEd a live, aggregated view of enrolled EV charging as coordinated, dispatchable grid capacity. It's the active managed-charging layer Illinois law already requires ComEd to build, and one ComEd's own pilot work has already identified as the missing piece next to panel-level load management.
...
An electric grid has to keep supply and demand matched in real time to hold a stable frequency. Two things make that harder every year: demand spikes that weren't scheduled for, and supply that shows up or disappears on its own schedule, as wind and solar output shifts faster than dispatchable generation can follow. A utility carries real, ongoing cost for both sides of that problem: building and maintaining capacity margin, buying expensive last-minute supply when demand outruns forecast, and absorbing or shedding load when supply outruns it.

Aggregated, price-aware EV charging is one of the few levers that can move in either direction without new hardware in the field. Charging load can shift into a surplus-supply window, soaking up power the grid would otherwise waste or curtail, or out of a strained window, acting in effect like a battery the utility doesn't have to build. Here's what that maps to, and what's actually confirmed for ComEd versus true in general:
...
- **Distribution and transmission deferral.** Shifting charging away from local peaks can delay the substation and feeder upgrades a fixed load pattern would otherwise force. This is the same mechanism ComEd's own EV EMS pilot used to avoid $5,000 to $15,000 household panel upgrades, applied at the circuit and substation level instead of the panel level.[^2]
...
- A live view of enrolled EV charging as aggregated, flexible capacity, updated continuously.
...
The per-customer fee can now be grounded in two figures. No competitor publishes what it actually charges a utility, but a third-party industry guide, not a party selling into this deal, puts utility-deployed DR/DERMS SaaS platforms at **$5 to $20 per participant per year, plus implementation services**, the category Flexy's model matches, as distinct from an aggregator's revenue-share model.[^6] Checked against what ComEd would actually realize per enrolled EV ($145 to $575/year, per the Brattle/ev.energy avoided-cost figure already cited above), that benchmark represents roughly 1 to 14 percent of the value delivered per vehicle, a defensible share regardless of how many households ultimately enroll.[^6][^7]

That gives a workable **per-customer fee range of $5 to $20/year** to anchor a real negotiation, since the low and high ends both fall inside what comparable platforms already charge. What the total contract is worth to Flexy still depends on enrollment scale, which no pilot has established yet:
...
The annual platform license fee stays a separate, unbenchmarked component. No source found this pass prices that piece specifically, so it remains a number to set in direct negotiation with ComEd. If ComEd's own program design ties Flexy's fee to verified capacity delivered, a performance-based component rather than a flat per-participant rate, that's a real option worth raising directly.
...
- **Reporting completeness against ComEd's own compliance needs** (CRGA, BE Plan).

## Open questions

- What a ComEd-specific active-managed-charging program would actually be worth in dollars or MW is not calculated anywhere in this research. The panel-EMS pilot's avoided-cost figure ($5,000 to $15,000 per household) answers a different question. This is a real gap to close with ComEd directly.
...
- Which CRGA-aligned mechanism (VPP tariff, BE Plan program, a PJM-facing structure, or something ComEd designs) this offer should actually target is ComEd's call.

## Status
Draft v2. Positioning, the why-ComEd-cares argument, the WeaveGrid-relationship framing, offer structure, and partnership success metrics are drafted from `utility-offer-grounding-research.md`. The Business Model section is updated with a sourced per-customer fee range ($5 to $20/year) and a savings-to-ComEd model at three enrollment scenarios, replacing the earlier unset placeholder. The platform license fee remains explicitly unbenchmarked. The full KPI set, expanding the four partnership success metrics above, and screen-level IA now live in `utility-dashboard-kpi-framework.md`. The coded dashboard (`utility-dashboard.html`) is built. A `research-auditor` run has completed; see its findings for `utility-offer-grounding-research.md` and this document.

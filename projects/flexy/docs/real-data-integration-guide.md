...
- `type=daynexttomorrow` - functional but undocumented. Returns tomorrow's PJM day-ahead hourly prices in the same array-literal format as `type=day`, parsed with the same regex. This is the same data ComEd's own public "Day-Ahead Prices" page (hourlypricing.comed.com/live-prices/day-ahead-prices/) charts. Unlike the real-time feeds above, it isn't incremental: it's empty (`[]`) until ComEd posts the full 24-hour set for the next day, typically around 4:30pm CT, then stays fixed, it doesn't get revised through the evening the way real-time prices settle. The prototype's parser already rejects a response with fewer than 6 populated hours as unusable, which is exactly what an empty pre-4:30pm response looks like, so no separate "not posted yet" detection was needed, that check already existed for the `type=day` case where today's hours are still filling in.
...
**Worth naming explicitly, since it's the one place this integration diverges from how Tibber's day-ahead pricing works for its customers:** ComEd's day-ahead price is advisory, not billing. PJM's day-ahead market clears once per day and gives a real, published estimate of what tomorrow's real-time prices will likely be, but ComEd's residential Real-Time Pricing program still bills customers on the real-time price as it actually settles, hour by hour. Tibber's customers, by contrast, are billed on the Nord Pool/EPEX day-ahead price directly. The prototype's Tomorrow view says this explicitly next to the chart so it doesn't read as "here's what you'll pay tomorrow" when it's closer to "here's what ComEd expects tomorrow to look like."

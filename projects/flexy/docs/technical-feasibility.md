...
**Tested:** registered a real Smartcar application, connected a simulated Tesla through the actual Connect consent flow, and pulled real API responses (application access token, connection list, vehicle detail) back from Smartcar's live v3 API Authentication endpoints. "Real and live" describes the integration plumbing, not the car: the app registration, the Connect flow, and the token/connection/vehicle-detail responses all came from Smartcar's actual production servers, not a mock Flexy built. The vehicle on the other end was Smartcar's own developer simulator, a fake car Smartcar provides for testing, not a real physical Tesla. No real vehicle has been connected or controlled. One real implementation detail worth flagging for a future team: this app is provisioned on Smartcar's newer v3 model, application-level access token via OAuth2 Client Credentials, plus a per-user `sc-user-id` header, not the older per-vehicle authorization-code exchange most third-party guides (including Optiwatt's) still document. The two aren't interchangeable; picking the wrong one costs real debugging time, as it did here.
...
| Flexy is EV-brand agnostic | Proven - real Smartcar app, real Connect flow, real access token and vehicle data pulled from Smartcar's live API (Tesla tested via Smartcar's own vehicle simulator, not a real physical car; other brands rely on Smartcar's own multi-brand support, not independently re-tested per brand) |
| Flexy can actually start/stop a vehicle's charging remotely | Not yet proven - Smartcar's API documents this as a supported capability, but no call has been made to Smartcar's charge-control endpoints against a real or simulated vehicle. Everything tested so far (Connect flow, access token, connection list, vehicle detail) is authentication and read-only data, not control |
